...
def apply_rope(x: Tensor, positions: Tensor) -> Tensor:
    ...
    Args:
        x:         [B, H, S, d_head]
        positions: [B, S] float positions

    Returns:
        [B, H, S, d_head]  (same shape, rotated)
    """
    B, H, S, d = x.shape
    half = d // 2

    theta = 1.0 / (10000.0 ** (torch.arange(0, half, device=x.device, dtype=x.dtype) / half))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        attn = torch.matmul(Q, K.transpose(-2, -1)) / scale  # [B, H, Sq, Sk]
...
class EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = self.res1(x, lambda z: self.self_attn(z, z, z, mask, positions))
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x = self.res1(x, lambda z: self.self_attn(z, z, z, tgt_mask, positions))

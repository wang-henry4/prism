"""
Train the ForwardModel (thin-film structure → optical spectrum).
...
from optoformer.training.train import make_optimizer_and_scheduler, train_forward


def main() -> None:
    parser = argparse.ArgumentParser(description="Train ForwardModel")
    ...
    parser.add_argument("--run_name",    default="forward_v1")
    parser.add_argument("--save_dir",    default="./saved_models/forward")
    ...
    model = make_forward_model(len(vocab), config).to(device)
